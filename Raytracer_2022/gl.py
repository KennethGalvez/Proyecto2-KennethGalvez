    ...
    def cast_ray(self, orig, dir, sceneObj = None, recursion = 0):
        ...
        if material.matType == OPAQUE:
            for light in self.lights:
                ...
                finalColor = mt.sumaV(finalColor, lightColor)
            ...
            specColor = [0,0,0]
            for light in self.lights:
                specColor = mt.sumaV(specColor, light.getSpecColor(intersect, self))

            finalColor = mt.sumaV(reflectColor, specColor)
        
        elif material.matType == TRANSPARENT:
            outside = mt.productoV(dir, intersect.normal) < 0 
            ...
            reflectOrigin = mt.sumaV(intersect.point, bias) if outside else mt.restaV(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrigin = mt.restaV(intersect.point, bias) if outside else mt.sumaV(intersect.point, bias)
                refractColor = self.cast_ray(refractOrigin, refract, None, recursion + 1)
            
            #finalColor = reflectColor * kr + refractColor * (1-kr) + specColor
            finalColor = mt.sumaV(mt.sumaV([mt.multiplicacionV(kr, refractColor)], [mt.multiplicacionV([1-kr], refractColor)]), specColor)

        finalColor = mt.multiV(objectColor, finalColor)
        ...
        if material.texture and intersect.texcoords:
            ...
            if texColor is not None:
                finalColor = mt.multiV(texColor, finalColor) 
    ...
    def glRender(self):
        ...
        for y in range(self.vpY, self.vpY + self.vpHeight + 1, STEPS):
            for x in range(self.vpX, self.vpX + self.vpWidth + 1, STEPS):
                ...
                direction = mt.normalizarV(direction)
